...
export default function RolePromptBuilder() {
  ...
  return (
    <ToolShell title="Rollen-Prompt-Builder" description="Bau einen Rollen-Dialog mit einer historischen Figur. Die Vorlage baut Caveats automatisch ein — und am Ende erinnern wir dich an die drei didaktischen Regeln.">
      <div>
        <label htmlFor={figureId} className="block text-sm font-semibold text-text-primary">Historische Figur</label>
        <input id={figureId} type="text" value={figure} onChange={(e) => setFigure(e.target.value)} placeholder="z.B. Maximilien de Robespierre, Goethe, Marie Curie" className={fieldClass} />
      </div>

      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <label htmlFor={yearId} className="block text-sm font-semibold text-text-primary">Jahr / Zeitpunkt</label>
          <input id={yearId} type="text" value={year} onChange={(e) => setYear(e.target.value)} placeholder="z.B. 1794, Mai" className={fieldClass} />
        </div>
        <div>
          <label htmlFor={audienceId} className="block text-sm font-semibold text-text-primary">Gesprächspartner:innen</label>
          <input id={audienceId} type="text" value={audience} onChange={(e) => setAudience(e.target.value)} className={fieldClass} />
        </div>
      </div>

      <div className="mt-4">
        <label htmlFor={styleId} className="block text-sm font-semibold text-text-primary">Stil</label>
        <input id={styleId} type="text" value={style} onChange={(e) => setStyle(e.target.value)} placeholder="z.B. ernsthaft, oft pathetisch, mit Verweisen auf Tugend" className={fieldClass} />
        <div className="mt-2 flex flex-wrap gap-1.5">
          {STYLE_PRESETS.map((s) => (
            <button key={s} type="button" aria-pressed={style === s} onClick={() => setStyle(s)} className="rounded-full border border-border-secondary bg-background-secondary px-3 py-1 text-xs text-text-secondary transition hover:border-purple hover:text-purple">
              {s}
            </button>
          ))}
        </div>
